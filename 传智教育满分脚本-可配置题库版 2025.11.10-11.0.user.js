...
// @version      11.0
// @description  ✅ 修复按钮 + 可配置题库 + 言溪题库标准接口
// @author       Claude优化版
...
(function() {
    ...
    function initUI() {
        ...
        cfg.innerHTML = `
            ...
            <div class="cfg-section">
                <h3>🤖 AI配置（兜底）</h3>
                <label>
                    <input type="checkbox" id="gpt_sw" ${CONFIG.gpt_enabled ? 'checked' : ''}>
                    启用GPT兜底（题库找不到时使用）
                </label>
                <input type="text" id="gpt_k" placeholder="粘贴你的 GPT API Key" value="${CONFIG.gpt_key}">
                <input type="text" id="gpt_u" placeholder="GPT API URL（默认即可）" value="${CONFIG.gpt_url}">
            </div>
    ...
    function renderBanksList() {
        ...
        list.innerHTML = CONFIG.banks.map((bank, index) => `
            <div class="bank-item ${!bank.enabled ? 'disabled' : ''}" data-index="${index}">
                <label>
                    <input type="checkbox" class="bank-toggle" data-index="${index}" ${bank.enabled ? 'checked' : ''}>
                ...
                <input type="password" placeholder="Token/Key（如有）" value="${bank.token || ''}"
    ...
    function extractOptions(element) {
        const optionElements = element.querySelectorAll('.option, .radio_item, label');
        const options = [];
    ...
    function detectQuestionType(element) {
        ...
        if (text.includes('单选') || element.querySelectorAll('input[type="radio"]').length > 0) {
            return '0';
        }
        if (text.includes('多选') || element.querySelectorAll('input[type="checkbox"]').length > 0) {
            return '1';
        }
        if (text.includes('判断')) {
            return '3';
        }
        if (text.includes('填空') || element.querySelectorAll('input[type="text"]').length > 0) {
            return '2';
        }
        return '0';
    }
    ...
    function queryBanks(question, options, type, element, num, total) {
        ...
        function tryNext() {
            ...
            GM_xmlhttpRequest({
                ...
                data: bank.method === 'POST' ? JSON.stringify({
                    token: bank.token || '',
                    title: question,
                    options: options,
                    type: type
    ...
    function fillAnswer(element, answer, source, num) {
        ...
        // 处理选择题
        const options = element.querySelectorAll('label, .option, .radio_item, input[type="radio"], input[type="checkbox"]');
        options.forEach(option => {
            let optionText = (option.innerText || option.value || '').trim();
            optionText = optionText.replace(/^[A-Z]\.?\s*/, '').replace(/[\s\n]+/g, '');

            answers.forEach(ans => {
                const cleanAns = ans.replace(/^[A-Z]\.?\s*/, '').replace(/[\s\n]+/g, '');

                if (optionText.includes(cleanAns) || cleanAns.includes(optionText) ||
